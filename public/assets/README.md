...
The supplied artwork lives here, background removed:

| File                          | Where it is used                                    |
| ----------------------------- | --------------------------------------------------- |
| `logo-only-removedbg.png`     | The mark in the nav bar and the menu takeover        |
| `nameonly-removedbg.png`      | The wordmark beside it, tagline already set into it  |
| `logo-only-removedbg.ico`     | Copied to `src/app/favicon.ico` — the browser tab    |

Both PNGs are gold on transparency, which is why there is no light/dark pair:
gold holds on the cream page and on the ink panels alike. Paths are declared in
[`src/data/assets.ts`](../../src/data/assets.ts) — **filenames are matched
exactly**, so renaming one here means editing that file in the same commit.

The favicon is a copy rather than a reference: Next only serves `favicon.ico`
from `src/app`, not from `public/assets`. Re-copy it if the artwork changes.

The Apple touch icon is still generated by `src/app/apple-icon.tsx`, which draws
the mark on an ink square. That is deliberate — iOS does not honour
transparency and would render these PNGs on black.
